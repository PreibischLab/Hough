...
public class GaussianBentLineds implements MTFitFunction {
	...
	private static final double Estartds(final double[] x, final double[] a, final double[] b) {
		...
		double slope = (maxVal[1] - minVal[1] -a[2 * ndims +3] * (maxVal[0] * maxVal[0] - minVal[0] * minVal[0])) / (maxVal[0] - minVal[0]);		
		
		double ds = Math.abs(a[2 * ndims]);

		double[] dxvector = { ds / Math.sqrt( 1 + slope * slope) , slope * ds/ Math.sqrt( 1 + slope * slope) + 2 * a[2 * ndims + 3] * x[0] };
	...
	private static final double Edsum(final double[] x, final double[] a, final double[] b) {
