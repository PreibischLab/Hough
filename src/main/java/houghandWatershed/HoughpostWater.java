...
public class HoughpostWater {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int index = 0; index < correctparamlist.size(); ++index) {
			...
			System.out.println(" Label : " + correctparamlist.get(index).Label + " Amp: "
					+ correctparamlist.get(index).Intensity + " " + "Mu X: "
					+ correctparamlist.get(index).centroid.getDoublePosition(0) + " " + "Mu Y: "
					+ correctparamlist.get(index).centroid.getDoublePosition(1) + " " + "Sig X: "
					+ correctparamlist.get(index).sigmaX + " " + "Sig Y: " + correctparamlist.get(index).sigmaY);
		...
		// Determine starting and end points by giving the start and end
		// positions along the line and then doing a half Gaussian mask fit
