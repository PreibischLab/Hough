...
public class OverlayLines {
	...
	public static void OverlayObject(RandomAccessibleInterval<FloatType> inputimg,ArrayList<Lineobjects> linelist ){
		...
		Line newline = new Line(minX,
				...
				maxX,
				rho / Math.sin(Math.toRadians(theta))
						- maxX/ Math.tan(Math.toRadians(theta)));
	...
	public static void Overlay(RandomAccessibleInterval<FloatType> inputimg, ArrayList<RefinedPeak<Point>> SubpixelMinlist, double[] sizes,
			double[] min, double[] max, int label, long minX, long maxX) {
		...
		for (int index = 0; index < SubpixelMinlist.size(); ++index) {
			...
			Line newline = new Line(minX,
					points[1] / Math.sin(Math.toRadians(points[0]))- minX/ Math.tan(Math.toRadians(points[0])),
					maxX,
					points[1] / Math.sin(Math.toRadians(points[0]))
							- (maxX)/ Math.tan(Math.toRadians(points[0])));
	...
	public static void OverlayExactline(RandomAccessibleInterval<FloatType> inputimg,
		...
		if (maxcount > 0) {
			...
			// System.out.println("All: "+ " Found Peaks at :" + "Theta: " +
			// points[0] + " Rho: " + points[1]);
